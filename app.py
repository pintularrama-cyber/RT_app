...
# --- 1. PARCHES DE COMPATIBILIDAD IA (NINJA PATCHES) ---
...
def load_and_preprocess_data(file):
    ...
    # Regla consolidada: Exclusión de Plástico
...
class RTOptimizerEngine:
    ...
    def _assign_dynamic_blocks(self, df):
        """Lógica estable para mantener el conteo exacto de lotes."""
    ...
    def get_lot_audit(self, df):
        ...
        # Limpieza numérica comas a puntos para la IA
        ...
        cols_order = ['Status', 'Lot_ID', 'Subcontractor', 'Total_Joints', 'Current_Done', 'Done_%', 
                      'RT1_Rej', 'RT2_Pend', 'RT_Req', 'Required', 'Deficit', 'Welder', 'Process', 'Material', 'Block_Start']
    ...
    def execute_optimization(self, df_audit_base, audit):
        ...
        while sum(debts.values()) > 0 and not candidates.empty:
            def calc_impact(row):
                ...
                impact += row.get('AI_Prob', 0.0) # IA como desempate
                return impact
...
if uploaded_file:
    ...
    if audit_df.empty: st.warning("No sampling data found.")
        ...
        with tab2:
            st.subheader(f"Status Overview: {selected_sub}")
            ...
            # --- CONSULTA DE LOTE CONCRETO (RESTABLECIDA) ---
            if event.selection.rows:
                ...
                st.info("💡 Click on any row in the log table above to see individual joints.")
else:
