...
class RTOptimizerEngine:
    ...
    def get_lot_audit(self, df):
        ...
        # --- ETIQUETADO DE INTEGRIDAD (SIN APPLY/GROUPNAME PARA EVITAR ERRORES) ---
        df_wb = df_wb.sort_values(['Lot_ID', 'RTDate1', 'Dateofweld'], ascending=[True, True, True])
        
        # Pre-calculamos fallos por lote
        audit_counts = df_wb.groupby('Lot_ID').agg(
        ...
        # Unimos conteos a la tabla base para etiquetar
        df_wb = df_wb.merge(audit_counts[['Lot_ID', 'Rej_Count']], on='Lot_ID', how='left')
        
        inspection_types = []
        inspection_status = []
        
        # Procesamos etiquetas por fila
        current_lot = ""
        found_fail = False
        tracers = 0
        is_full_audit = False
        ...
        for _, row in df_wb.iterrows():
            if row['Lot_ID'] != current_lot:
                current_lot = row['Lot_ID']
                found_fail = False
                tracers = 0
                is_full_audit = row['Rej_Count'] > 1 # Si hay más de un fallo en el lote total
            ...
            inspection_status.append(s)

            # Type (7, 8, 12)
            if is_full_audit:
                inspection_types.append("PENALTY LOT")
            elif row['RT1rej'] and not found_fail:
                inspection_types.append("Random Inspection Joint") # El disparador
                found_fail = True
            elif found_fail and tracers < 2:
                inspection_types.append("PENALTY TRACER")
                tracers += 1
            else:
                inspection_types.append("Random Inspection Joint")

        df_wb['Inspection_Type'] = inspection_types
        df_wb['Inspection_Status'] = inspection_status
        ...
        audit_counts['Deficit'] = (audit_counts['Required'] - audit_counts['RT1_Done']).clip(lower=0)
        audit_counts['Done_%'] = (audit_counts['RT1_Done'] / audit_counts['Total_Joints'] * 100).round(1)
        audit_counts['Status'] = np.where((audit_counts['Deficit'] == 0) & (audit_counts['Pending_Repairs'] == 0), '🟢 CLOSED', '🔴 OPEN')
...
# --- UI ---
st.set_page_config(page_title="RT Optimizer 5.0", layout="wide", page_icon="🏗️")
...
if uploaded_file:
    ...
    st.table(pd.DataFrame({'Mandatory Column': REQUIRED_COLS}))