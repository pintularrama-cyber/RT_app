...
class RTOptimizerEngine:
    ...
    def get_lot_audit(self, df):
        ...
        # --- BLOQUES DINÁMICOS Y ID DE LOTE ---
        ...
        for _, group in df_filtered.groupby(self.lot_criteria, dropna=False):
            ...
            # Crear Lot_ID aquí mismo para que sea parte de la estructura del chunk
        ...
        # Unimos para tener Rej_Count disponible en cada junta
        ...
        def label_rows(group):
            ...
            n_rej_total = group['Rej_Count'].iloc[0]
            types = []; stats = []
            ...
            for _, row in group.iterrows():
                ...
                # Type (7, 8, 12)
                if n_rej_total > 1: types.append("Penalty Lot 100%")
                elif n_rej_total == 1:
...
if uploaded_file:
    ...
    if audit_df.empty: st.warning("No sampling data found.")
        ...
        with tab2:
            ...
            k3.metric("Project Compliance", f"{( (total_l - open_l)/total_l*100 if total_l>0 else 0):.1f}%")
